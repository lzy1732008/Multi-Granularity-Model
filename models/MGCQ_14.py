...
class MultiGranularityCNNModel:
    ...
    def build_model(self):
        with tf.variable_scope("first-encoder"):
            self.input_x1_encoder_1 = self.selfAttention(self.input_X1)  #获取内在的特征，寻找关键词
            self.input_x2_encoder_1 = self.selfAttention(self.input_X2)

        with tf.variable_scope("first-CNN-layer"):
            self.output_x1_1 = tf.layers.conv1d(self.input_x1_encoder_1,filters=self.config.filters_num,kernel_size=self.config.first_kernel_size,padding='same',name='first-cnn1')
            self.output_x2_1 = tf.layers.conv1d(self.input_x2_encoder_1,filters=self.config.filters_num,kernel_size=self.config.first_kernel_size,padding='same',name='first-cnn2')

        with tf.variable_scope("first-interaction"):
            self.inter1_output_x2 = self.interaction(self.output_x1_1,self.output_x2_1)

        with tf.variable_scope("second-encoder"):
            self.input_x1_encoder_2 = self.selfAttention(self.output_x1_1)  #获取内在的特征，寻找关键词
            self.input_x2_encoder_2 = self.selfAttention(self.output_x2_1)

        with tf.variable_scope("second-CNN-layer"):
            self.output_x1_2 = tf.layers.conv1d(self.input_x1_encoder_2,filters=self.config.filters_num,kernel_size=self.config.second_kernel_size,padding='same',name='second-cnn1')
            self.output_x2_2 = tf.layers.conv1d(self.input_x2_encoder_2,filters=self.config.filters_num,kernel_size=self.config.second_kernel_size,padding='same',name='second-cnn2')
        ...
        with tf.variable_scope("fusion-layer"):
            self.fusion_output = tf.concat([self.inter1_output_x2, self.inter2_output_x2], axis=-1)  # [Batch, 3 * len]
    ...
    def selfAttention(self,input):
        Q = tf.layers.dense(input, param.BaseConfig.word_dimension, activation=tf.nn.relu)  # (N, T_q, C)
        K = tf.layers.dense(input, param.BaseConfig.word_dimension, activation=tf.nn.relu)  # (N, T_k, C)
        V = tf.layers.dense(input, param.BaseConfig.word_dimension, activation=tf.nn.relu)  # (N, T_k, C)
