...
class MultiGranularityCNNModel:
    ...
    def build_model(self):
        ...
        with tf.variable_scope("second-CNN-layer"):
            self.output_x1_2 = tf.layers.conv1d(tf.concat([self.input_X1, self.output_x1_1],axis=-1),filters=self.config.filters_num,kernel_size=self.config.second_kernel_size,padding='same',name='second-cnn1')
            self.output_x2_2 = tf.layers.conv1d(tf.concat([self.input_X2, self.output_x2_1],axis=-1),filters=self.config.filters_num,kernel_size=self.config.second_kernel_size,padding='same',name='second-cnn2')
        ...
        with tf.variable_scope("third-CNN-layer"):
            self.output_x1_3 = tf.layers.conv1d(tf.concat([self.input_X1, self.output_x1_2],axis=-1), filters=self.config.filters_num,
                                                kernel_size=self.config.third_kernel_size, padding='same',
                                                name='second-cnn1')
            self.output_x2_3 = tf.layers.conv1d(tf.concat([self.input_X2, self.output_x2_2],axis=-1), filters=self.config.filters_num,
                                                kernel_size=self.config.third_kernel_size, padding='same',
                                                name='second-cnn2')
