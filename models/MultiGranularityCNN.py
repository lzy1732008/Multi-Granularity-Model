...
class MultiGranularityCNNModel:
    ...
    def build_model(self):
        with tf.variable_scope("first-CNN-layer"):
            self.output_x1_1 = tf.layers.conv1d(self.input_X1,filters=self.config.filters_num,kernel_size=2,padding='same',name='first-cnn1')
            self.output_x2_1 = tf.layers.conv1d(self.input_X2,filters=self.config.filters_num,kernel_size=2,padding='same',name='first-cnn2')

        with tf.variable_scope("first-interaction"):
            self.inter_output_1 = self.interaction(self.output_x1_1,self.output_x2_1)

        with tf.variable_scope("second-CNN-layer"):
            self.output_x1_2 = tf.layers.conv1d(self.output_x1_1,filters=self.config.filters_num,kernel_size=2,padding='same',name='second-cnn1')
            self.output_x2_2 = tf.layers.conv1d(self.output_x2_1,filters=self.config.filters_num,kernel_size=2,padding='same',name='second-cnn2')

        with tf.variable_scope("second-interaction"):
            self.inter_output_2 = self.interaction(self.output_x1_2,self.output_x2_2)
