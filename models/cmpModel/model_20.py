...
class MultiGranularityCNNModel:
    ...
    def build_model(self):
        with tf.variable_scope("zero-interaction-layer"):
            self.beta_1 = tf.Variable(tf.random_normal(shape=[2, 1], stddev=0, seed=1, dtype=tf.float32),
                                      trainable=True,name='beta1')
            play_inter_0 = Interaction(6, self.input_X1, self.input_X2, self.x2_label, self.beta_1)
            self.inter_0 = play_inter_0.exeInteraction()
        ...
        with tf.variable_scope("fusion-layer-0"):
            ...
            self.fusion_output_0 = tf.concat(
                [self.input_X2, self.x2_inter_0, self.input_X2 - self.x2_inter_0, self.input_X2 * self.x2_inter_0,self.x2_label],
        ...
        with tf.variable_scope("first-interaction"):
            self.beta_2 = tf.Variable(tf.random_normal(shape=[2, 1], stddev=0, seed=1, dtype=tf.float32),
                                      trainable=True, name='beta2')
            play_inter_1 = Interaction(6, self.output_x1_1, self.output_x2_1, self.x2_label, self.beta_2)
            self.inter_1 = play_inter_1.exeInteraction()
        ...
        with tf.variable_scope("fusion-layer-1"):
            ...
            self.fusion_output_1 = tf.concat(
                [self.input_X2, self.x2_inter_1, self.input_X2 - self.x2_inter_1, self.input_X2 * self.x2_inter_1, self.x2_label],
                axis=-1)  # [Batch, len, 4 * dimension]
        ...
        with tf.variable_scope("second-interaction"):
            self.beta_3 = tf.Variable(tf.random_normal(shape=[2, 1], stddev=0, seed=1, dtype=tf.float32),
                                      trainable=True, name='beta3')
            play_inter_2 = Interaction(6, self.output_x1_2, self.output_x2_2, self.x2_label, self.beta_2)
            self.inter_2 = play_inter_2.exeInteraction()
        ...
        with tf.variable_scope("fusion-layer-3"):
            ...
            self.fusion_output_3 = tf.concat([self.input_X2,self.x2_inter_3,self.input_X2 - self.x2_inter_3, self.input_X2 * self.x2_inter_3, self.x2_label], axis=-1)  # [Batch, len, 2 + 4 * dimension]
