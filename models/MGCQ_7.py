    ...
    def build_model(self):
        with tf.variable_scope("first-CNN-layer"):
            ...
            self.output_x2_1 = tf.layers.conv1d(self.input_X2, filters=self.config.filters_num,
                                                kernel_size=self.config.first_kernel_size, padding='same',
                                                name='first-cnn2')
            self.pooling_1 = tf.reduce_max(self.output_x2_1,axis=-1)
        ...
        with tf.variable_scope("second-CNN-layer"):
            ...
            self.output_x2_2 = tf.layers.conv1d(self.output_x2_1, filters=self.config.filters_num,
                                                kernel_size=self.config.second_kernel_size, padding='same',
                                                name='second-cnn2')
            self.pooling_2 = tf.reduce_max(self.output_x2_2, axis=-1)
        ...
        with tf.variable_scope("third-CNN-layer"):
            ...
            self.output_x2_3 = tf.layers.conv1d(self.output_x2_2, filters=self.config.filters_num,
                                                kernel_size=self.config.third_kernel_size, padding='same',
                                                name='third-cnn2')
            self.pooling_3 = tf.reduce_max(self.output_x2_3, axis=-1)
        ...
        with tf.variable_scope("fusion-layer"):
            self.inter1 = tf.nn.relu(tf.layers.dense(tf.concat([self.inter1_output_x2,self.pooling_1],axis=-1),units=64))
            self.inter2 = tf.nn.relu(
                tf.layers.dense(tf.concat([self.inter2_output_x2, self.pooling_2], axis=-1), units=64))
            self.inter3 = tf.nn.relu(
                tf.layers.dense(tf.concat([self.inter3_output_x2, self.pooling_3], axis=-1), units=64))
            self.fusion_output = tf.concat([self.inter1,self.inter2,self.inter3],axis=-1)
